...
export function FilesPane({
  ...
  const [confirmingKeyDelete, setConfirmingKeyDelete] = useState<Set<string> | null>(null)
  const [confirmingContextDelete, setConfirmingContextDelete] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    const dismiss = () => {
      setContextMenu(null)
      setConfirmingContextDelete(null)
    }
  ...
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      ...
      if ((e.key === 'Backspace' || e.key === 'Delete') && selected.size > 0) {
        e.preventDefault()
        setConfirmingKeyDelete(new Set(selected))
      }

      if (e.key === 'Escape') {
        setConfirmingKeyDelete(null)
      }
    }
  ...
  function handleContextMenuDelete() {
    ...
    setConfirmingContextDelete(contextMenu.id)
  }
  ...
  return (
    ...
    >
      ...
      {/* Keyboard-delete confirmation banner */}
      {confirmingKeyDelete && (
        <div className="mx-4 mb-2 flex items-center gap-2 px-3 py-2 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-lg">
          <span className="text-[12px] text-red-600 dark:text-red-400 flex-1">
            Delete {confirmingKeyDelete.size} {confirmingKeyDelete.size === 1 ? 'file' : 'files'}?
          </span>
          <button
            className="px-2.5 py-1 rounded text-[11px] font-medium bg-red-500 text-white hover:bg-red-600 transition-colors"
            onClick={() => {
              for (const id of confirmingKeyDelete) {
                onDelete(id)
              }
              setSelected(new Set())
              lastSelectedRef.current = null
          ...
          <button
            ...
            onClick={() => setConfirmingKeyDelete(null)}
          >
            Cancel
          </button>
        </div>
      ...
      {contextMenu && (
        ...
        >
          ...
          {contextMenu.kind === 'file' && (
            confirmingContextDelete === contextMenu.id ? (
              <div className="flex items-center gap-1.5 px-3 py-2">
                <span className="text-[12px] text-red-500 dark:text-red-400 mr-1">Delete?</span>
                <button
                  className="px-2 py-1 rounded text-[11px] font-medium bg-red-500 text-white hover:bg-red-600 transition-colors"
                  onClick={confirmContextDelete}
                >
                  Delete
                </button>
                <button
                  className="px-2 py-1 rounded text-[11px] font-medium text-neutral-500 hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-colors"
                  onClick={() => setConfirmingContextDelete(null)}
                >
                  Cancel
                </button>
              </div>
